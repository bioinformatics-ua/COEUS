<%@include file="taglib.jsp" %>
<!--<link href='http://fonts.googleapis.com/css?family=Open+Sans:300italic,700italic,300,700' rel='stylesheet' type='text/css'>-->
<link rel="stylesheet" href="<c:url value="/assets/css/bootstrap3.css" />"/>
<link rel="stylesheet" href="<c:url value="/assets/css/setup.css" />"/>
<link rel="stylesheet" href="<c:url value="/assets/css/typeahead.css" />"/>
<link rel="stylesheet" href="<c:url value="/assets/css/font-awesome.min.css" />"/>
<!--[if IE 7]>
<link rel="stylesheet" href="assets/css/fontawesome-ie7.css">
<![endif]-->